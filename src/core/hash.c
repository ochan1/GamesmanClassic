...
void generic_hash_hash_sym_thread(t_args_t* args) {
	int i,j;
	POSITION temp;
	POSITION minPos = -1;
	...
	if (minPos == -1 || temp < minPos) {
		minPos = temp;
		//memcpy(cCon->)
	}
	//pthread_mutex_unlock(&lock);
	pthread_exit((void*) minPos);
}
...
POSITION hash_cruncher_sym_threads(char* board, struct symEntry* sym, int* counts, int* mins) {
	POSITION sum = 0;
	int i = 0, k = 0, max1 = 0;
	for (int boardSize = cCon->boardSize; boardSize>1;boardSize--) {
		i = 0;
		while (board[sym->sym[boardSize-1]] != cCon->pieces[i]) {
			i++;
		}
		for (k = 0; k < i; k++) {
			max1 = 1;
...
void generic_hash_init_sym(int boardType, int numRows, int numCols, int* reflections, int numReflects, int* rotations, int numRots, int flippable) {
	...
	for (symIndex = symmetriesList; symIndex != NULL; symIndex = symIndex->next) {
		...
		args->localMins = SafeMalloc(sizeof(int)*cCon->numPieces);	
		thread_args[i] = args;
		i++;
	}
	threads = SafeMalloc(sizeof(pthread_t)*numSymmetries);
	//#endif
...
POSITION generic_hash_canonicalPosition(POSITION pos) {
	...
	for (symIndex = symmetriesList; symIndex != NULL; symIndex = symIndex->next) {
		args = thread_args[i];
		if (symIndex->flip == 1) {
			memcpy(args->board,flippedboard,sizeof(char)*cCon->boardSize);
			args->player = flippedplayer;
			args->offset = flippedoffset;
		} else {
			memcpy(args->board,board,sizeof(char)*cCon->boardSize);
			args->player = player;
			args->offset = offset;
		}
		//args->minPos = &minPos;
		args->localCounts = realloc(args->localCounts,sizeof(int)*cCon->numPieces);
		args->localMins = realloc(args->localMins,sizeof(int)*cCon->numPieces);
		memset(args->localCounts,0,sizeof(int)*cCon->numPieces);
		memcpy(args->localMins, cCon->mins,sizeof(int)*cCon->numPieces);
		//pthread_create(&threads[i],NULL,(void*) generic_hash_hash_sym_thread, args);
		i++;
	}
	for (i = 0; i < numSymmetries; i++) {
	...
	void* t_min;
	for (i = 0; i < numSymmetries; i++) {
		pthread_join(threads[i],&t_min);
		if (i == 0 || (POSITION) t_min < minPos) {
			minPos = (POSITION) t_min;
		} 
	}
	//#endif
...
void freeSymmetries() {
	...
	while(symIndex != NULL) {
		...
		symIndex = tempNext;
		SafeFree(args->board);
		SafeFree(args->localCounts);
		SafeFree(args->localMins);
		i++;	
	}
